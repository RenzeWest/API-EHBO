...
const shiftController = {
	...
	getShiftInformationById: (req, res, next) => {
		...
		shiftService.getShiftInformationById(req.query.shiftId, req.userId, (error, succes) => {
			if (error) {
	...
	getAssignedShifts: (req, res, next) => {
		const projectId = req.query.projectId;
		logger.trace("shiftController -> getAssignedShifts" + projectId);

		if (!projectId) {
			return res.status(400).json({ message: "Missing project ID" });
		}

		shiftService.getAssignedShifts(projectId, (error, success) => {
			if (error) {
				return next({
					status: error.status,
					message: error.message,
